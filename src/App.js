...
const App = () => {
    ...
    fetchCodes(data).then((response) => {
      if (response.status === undefined) {
        if (response.number_uses < response.discount_code_template.use_limit) {
          setState(2);
          setCantidad(
            response.discount_code_template.use_limit - response.number_uses
          );
        }
      } else {
          ...
          <div style={{ display: "block" }}>
            <div style={{ display: "block", paddingBottom: "10px" }}>
              ...
              <Input
                ...
                type="text"
                value={code}
                onChange={(event) => helperChange(event)}
              />
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <Button onClick={() => handleCancel()}>Cancelar</Button>
              <Button id="buttonUsar" onClick={() => handleOk()}>Usar</Button>
            </div>
          </div>
        </div>
      </div>
  ...
  const disponible = () => {
    return (
      <div id="divDisponible" style={{ display: "block" }}>
        <div style={{ display: "flex", justifyContent: "center" }}>
  ...
  const Usado = () => {
    return (
      <div id="divUsadoInvalido" style={{ display: "block" }}>
        <div style={{ display: "flex", justifyContent: "center" }}>
          <Avatar size={64} icon={<AuditOutlined />} />
        </div>
        <div
        ...
        >
          <div
            style={{ display: "block", justifyContent: "center", width: "50%" }}
          >
            <h2 style={{ textAlign: "center", margin: 0 }}>
              {state === 3 ? "Código usado" : "Código invalido"}
            </h2>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <Button onClick={() => handleCancel()}>Cancelar</Button>
